...
def get_scatter_chart(site_value, payload_value):
    ...
    if site_value == 'ALL':
        filtered_df = spacex_df[['class', "Payload Mass (kg)", 'Booster Version Category']]
        ...
        filtered_df = filtered_df[['class', "Payload Mass (kg)", 'Booster Version Category']]
        filtered_df = filtered_df[(filtered_df["Payload Mass (kg)"] >= low) & (filtered_df["Payload Mass (kg)"] <= high)]
        fig = px.scatter(filtered_df, x="Payload Mass (kg)", 
                            y='class', 
                            color='Booster Version Category',
                            title="Correlation between Payload and Success for {}".format(site_value))
        return fig
